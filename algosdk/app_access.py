...
def translate_to_resource_references(
        ...
        holdings: List[Tuple[int, str]]=None,
        locals: List[Tuple[int, str]]=None
    ...
    def ensure(target: "ResourceReference") -> int:
        for idx, a in enumerate(access):
            if a.address == target.address and a.asset_id == target.asset_id and a.app_id == target.app_id:
                return idx + 1
    ...
    for asset, addr in holdings or []:
        ...
        access.append(ResourceReference(holding_reference=HoldingRef(asset_index=asset_idx, address=addr_idx)))
    ...
    for app, addr in locals or []:
        ...
        if addr and addr != "":
            addr_idx = ensure(ResourceReference(address=addr))
        access.append(ResourceReference(locals_reference=LocalsRef(app_index=app_idx, addr_index=addr_idx)))
    ...
    for app, name in boxes or []:
        ...
        if app and app != 0 and app != app_id:
            app_idx = ensure(ResourceReference(app_id=app))
        access.append(ResourceReference(box_reference=BoxReference(app_index=app_idx, name=name)))

    return access


class ResourceReference:
    def __init__(self, address: str=None, asset_id: int=None, app_id: int=None, box_reference: BoxReference=None, holding_reference: "HoldingRef"=None, locals_reference: "LocalsRef"=None):
        self.app_id = app_id
    ...
    def undictify(d):
        return ResourceReference(
            ...
            holding_reference=HoldingRef.undictify(d["h"]) if "h" in d else None,
...
class LocalsRef:
    ...
    def __eq__(self, other):
        if not isinstance(other, LocalsRef):
            return False
        return self.app_index == other.app_index and self.addr_index == other.addr_index
...
class HoldingRef:
    ...
    def __eq__(self, other):
        if not isinstance(other, HoldingRef):
            return False
        return self.asset_index == other.asset_index and self.addr_index == other.addr_index
