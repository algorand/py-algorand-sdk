...
def parse_pending_txn(context, sender):
    ...
    assert (
        encoding.encode_address(base64.b64decode(context.response["txn"]["txn"]["snd"]))
        == sender
    )
...
def asset_balance(
    context, index, limit, afterAddress, block, currencyGreaterThan, currencyLessThan
):
...
def lookup_account_check_created(context, num, index, name, total, unit):
    ...
    assert (
        context.response["account"]["created-assets"][0]["params"]["unit-name"] == unit
    )
    assert context.response["account"]["created-assets"][0]["params"]["total"] == int(
        total
    )
...
def search_accounts(
    context, index, limit, currencyGreaterThan, currencyLessThan, block, authAddr
):
    if authAddr == "none":
        authAddr = None
    context.response = context.icl.accounts(
...
def check_search_accounts(
    ...
    assert context.response["accounts"][0]["amount-without-pending-rewards"] == int(
        withoutrewards
    )
...
def check_search_accounts_online(
    ...
    assert context.response["accounts"][0]["address"] == address
    assert context.response["accounts"][0]["participation"]["vote-key-dilution"] == int(
        keydilution
    )
    assert context.response["accounts"][0]["participation"]["vote-first-valid"] == int(
        firstvalid
    )
    assert context.response["accounts"][0]["participation"]["vote-last-valid"] == int(
        lastvalid
    )
    assert (
        context.response["accounts"][0]["participation"]["vote-participation-key"]
        == votekey
    )
    assert (
        context.response["accounts"][0]["participation"]["selection-participation-key"]
        == selectionkey
    )
...
def check_assetid(context, assetid):
    for txn in context.response["transactions"]:
        ...
        assert subtxn["asset-id"] == int(assetid) or txn["created-asset-index"] == int(
            assetid
        )
...
def create_transient_and_fund(context, transient_fund_amount):
    ...
    payment = transaction.PaymentTxn(
        context.accounts[0], sp, context.transient_pk, int(transient_fund_amount)
    )
...
def sign_submit_save_txid_with_error(context, error_string):
    try:
        ...
        if error_string not in str(e):
            raise RuntimeError(
                "error string " + error_string + " not in actual error " + str(e)
            )
...
def verify_app_txn(
    ...
    if application_state == "local":
        ...
        raise NotImplementedError(
            'test does not understand application state "' + application_state + '"'
        )
...
@then('the parsed Dryrun Response should have global delta "{creator}" with {action}')
